...
class ScopeHandler:
    def __init__(self, func:ir.Function, scopes=[dict()]) -> None:
        self.scopes: List[Dict[str, ir.Constant]] = scopes
        self.func = func
        self.blocks = []
        self.builders = []
        self._eblox=0
        ...
        self._loopblox=0
        self._i = len(scopes)
    def get(self, var: str) -> ir.Constant:
    ...
    def set(self, var:str, value: ir.Constant):
        for i in range(len(self.scopes)-1, -1, -1):
            match self.scopes[i].get(var, None):
                case None:
                    pass
    ...
    def add_cond_block(self) -> ir.IRBuilder:
        block = self.func.append_basic_block(f"entry{self._condblox}")
        builder = ir.IRBuilder(block)
        self.blocks.append(block)
        self.builders.append(builder)
        self._condblox+=1
    ...
    def add_anon_block(self) -> ir.IRBuilder:
        block = self.func.append_basic_block(f"entry{self._anonblox}")
        builder = ir.IRBuilder(block)
        self.blocks.append(block)
        self.builders.append(builder)
        self._anonblox+=1
    ...
    def add_loop_block(self) -> ir.IRBuilder:
        block = self.func.append_basic_block(f"entry{self._loopblox}")
        builder = ir.IRBuilder(block)
        self.blocks.append(block)
        self.builders.append(builder)
        self._loopblox+=1
...
class Function:
    __slots__ = {'arg_t', 'fnptr', 'type', 'scopehnd', 'name'}
    def __init__(self, module, name, arg_t, type, parent_scopehnd=None) -> None:
        self.arg_t = arg_t
        self.type = type
        self.name = name
